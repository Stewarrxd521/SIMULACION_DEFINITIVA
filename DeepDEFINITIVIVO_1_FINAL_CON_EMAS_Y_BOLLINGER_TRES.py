...
# ==================== BAIL-OUT INTELIGENTE (ProfitTargetManager) ====================
# Tres señales independientes detectan cuando el ciclo ya no va a evolucionar y es
# mejor salir antes de que la situación se deteriore.
#
# SEÑAL 1 — Drawdown desde el pico del ciclo
# Si el balance combinado (realizado + no realizado) alcanzó un máximo y luego cae
# BAILOUT_PEAK_DRAWDOWN_USD dolares desde ese maximo -> salir ordenadamente.
# Ej: ciclo llego a +$1.80 combinado, cae $0.80 -> sale en +$1.00.
BAILOUT_ENABLED                    = True   # Activar / desactivar todo el bail-out
BAILOUT_PEAK_DRAWDOWN_USD          = 0.80   # Caida maxima permitida desde pico del ciclo ($)
BAILOUT_PEAK_DRAWDOWN_MIN_PEAK_USD = 0.30   # El pico debe superar este valor para activarse
#
# SEÑAL 2 — Estancamiento: el ciclo no avanza
# Si en los ultimos BAILOUT_STAGNATION_MINUTES la ganancia combinada NO mejoro en al
# menos BAILOUT_STAGNATION_MIN_DELTA_USD -> el ciclo esta muerto -> salir.
BAILOUT_STAGNATION_MINUTES         = 45     # Ventana de tiempo para medir estancamiento
BAILOUT_STAGNATION_MIN_DELTA_USD   = 0.25   # Mejora minima requerida en esa ventana ($)
BAILOUT_STAGNATION_ONLY_IF_NEGATIVE = True  # Solo si ganancia actual < 0
#
# SEÑAL 3 — Reversión del PnL no realizado
# Si el PnL no realizado alcanzo un pico positivo y luego revierte mas de
# BAILOUT_UNREALIZED_REVERSAL_PCT % de ese pico -> las posiciones giran en contra.
BAILOUT_UNREALIZED_REVERSAL_PCT    = 60.0   # % de caida desde pico no-realizado
BAILOUT_UNREALIZED_MIN_PEAK_USD    = 0.50   # Pico minimo no-realizado para activar
#
# Proteccion general
BAILOUT_MIN_CYCLE_MINUTES          = 10     # Minutos minimos antes de que bail-out pueda activarse
BAILOUT_CHECK_INTERVAL_SEC         = 15     # Frecuencia de evaluacion (segundos)
# ====================================================================================
...
class BailoutReason:
    """Identificadores de razon de bail-out para logging y reporting."""
    PEAK_DRAWDOWN        = "PEAK_DRAWDOWN"         # Caida desde el pico del ciclo
    STAGNATION           = "STAGNATION"            # Sin progreso en ventana de tiempo
    UNREALIZED_REVERSAL  = "UNREALIZED_REVERSAL"   # PnL no-realizado revertio desde pico
    TARGET_WIN           = "TARGET_WIN"            # Target de ganancia alcanzado
    TARGET_LOSS          = "TARGET_LOSS"           # Perdida extrema


class ProfitTargetManager:
    """
    Gestor de targets de ganancia con bail-out inteligente de 3 señales.

    Señales de salida:
      1. TARGET_WIN / TARGET_LOSS : logica original (gana o pierde)
      2. PEAK_DRAWDOWN            : balance combinado cayo X$ desde su maximo
      3. STAGNATION               : sin mejora en N minutos (opcional: solo si negativo)
      4. UNREALIZED_REVERSAL      : PnL no-realizado revertio mas del X% desde su pico

    Uso:
        reached, reason = manager.is_target_reached()
        if reached:
            handle_closure(reason)

        bail, reason = manager.should_bailout()
        if bail:
            handle_bailout(reason)
    """
    ...
    def __init__(self,
        ...
        # ── Estado interno del ciclo para bail-out ────────────────────────────
        # Cada vez que se llama a reset_cycle_tracking() estos se reinician.
        self._cycle_start_time      : Optional[datetime] = None   # Inicio del ciclo actual
        self._peak_combined_gain    : float = 0.0                  # Mayor ganancia combinada vista
        self._peak_combined_time    : Optional[datetime] = None   # Cuando se alcanzo el pico
        self._peak_unrealized       : float = 0.0                  # Mayor PnL no-realizado visto
        # Historial deslizante de (timestamp, combined_gain) para detectar estancamiento
        # Guardamos una muestra cada BAILOUT_CHECK_INTERVAL_SEC aproximadamente.
        self._gain_history          : deque = deque(maxlen=int((BAILOUT_STAGNATION_MINUTES * 60)
                                                                // max(BAILOUT_CHECK_INTERVAL_SEC, 1) + 2))
        # Razon del ultimo bail-out evaluado (para logs detallados)
        self._last_bailout_eval     : Optional[str] = None
    ...
    def attach_bot(self, bot_instance):
        """Adjunta el bot despues de crear el manager."""
        with self.lock:
    ...
    def reset_cycle_tracking(self):
        """
        Reinicia todos los contadores de ciclo.
        Llamar al inicio de cada ciclo nuevo (despues de un cierre o cooldown).
        """
        with self.lock:
            self._cycle_start_time   = datetime.now()
            self._peak_combined_gain = 0.0
            self._peak_combined_time = datetime.now()
            self._peak_unrealized    = 0.0
            self._gain_history.clear()
            self._last_bailout_eval  = None
            logger.info("PTM: Ciclo reiniciado — contadores bail-out a cero.")

    def set_next_target(self):
        """Registra target alcanzado y reinicia para el siguiente ciclo."""
        with self.lock:
            self.target_history.append({
                'target'   : float(self.current_target),
                'timestamp': datetime.now()
            })
            self.current_target = self.base_amount
            logger.info(f"Nuevo target: ${self.current_target:.2f}")
            if len(self.target_history) >= 2:
                self._reset_for_new_cycle_internal()

    def reset_for_new_cycle(self):
        """Reinicia para un nuevo ciclo (llamada externa)."""
        with self.lock:
            self._reset_for_new_cycle_internal()

    def _reset_for_new_cycle_internal(self):
        """(Interna) resetea current_target y tracking."""
    ...
    def _get_unrealized_summary(self) -> Optional[Dict]:
        try:
            if not self.bot:
                return None
            fn = getattr(self.bot, 'compute_unrealized_pnl_summary', None)
            if fn is None:
                return None
            return fn()
        except Exception as e:
            logger.debug(f"PTM: Error obteniendo PnL no realizado: {e}")
            return None

    def _get_combined_gain(self) -> Optional[float]:
        """
        Retorna la ganancia combinada actual del ciclo:
            (balance_realizado + pnl_no_realizado) - daily_start_balance
        """
        try:
            if not self.bot:
                return None
            ...
            realized    = balance - daily_start

            unrealized = 0.0
            if self.consider_unrealized:
                summary = self._get_unrealized_summary()
                if summary:
                    key = 'combined_balance_net_est' if self.use_net_estimate else 'combined_balance_gross'
                    combined = summary.get(key)
                    if combined is not None:
                        return float(combined) - daily_start
            return realized
        except Exception as e:
            logger.debug(f"PTM: Error calculando combined_gain: {e}")
            return None

    def _get_current_unrealized(self) -> float:
        """Retorna solo el PnL no realizado neto estimado (o 0 si no disponible)."""
        try:
            summary = self._get_unrealized_summary()
            if not summary:
                return 0.0
            key = 'total_unrealized_net_est' if self.use_net_estimate else 'total_unrealized_gross'
            return float(summary.get(key, 0.0))
        except Exception:
            return 0.0

    def _update_cycle_state(self, combined_gain: float):
        """
        Actualiza los maximos del ciclo y registra una muestra en el historial.
        Debe llamarse cada vez que se evalua el bail-out.
        """
        now = datetime.now()

        # Inicializar el ciclo si es la primera llamada
        if self._cycle_start_time is None:
            self._cycle_start_time = now

        # Actualizar pico de ganancia combinada
        if combined_gain > self._peak_combined_gain:
            self._peak_combined_gain = combined_gain
            self._peak_combined_time = now

        # Actualizar pico de PnL no-realizado
        unrealized = self._get_current_unrealized()
        if unrealized > self._peak_unrealized:
            self._peak_unrealized = unrealized

        # Registrar muestra historica
        self._gain_history.append((now, combined_gain))

    # ─────────────────────────────────────────────────────────────────────────
    # Logica central: TARGET y BAIL-OUT
    # ─────────────────────────────────────────────────────────────────────────

    def is_target_reached(self) -> tuple:
        """
        Evalua si se alcanzo el target de ganancia o la perdida extrema.

        Returns:
            (bool, str | None)  — (True, razon) si se activa, (False, None) si no.

        Compatibilidad: el return sigue siendo truthy/falsy por lo que el codigo
        existente que hace `if manager.is_target_reached():` sigue funcionando.
        Pero ahora puedes hacer: reached, reason = manager.is_target_reached()
        """
        with self.lock:
            try:
                if not self.bot:
                ...
                logger.debug(f"PTM: balance={balance:.2f}, daily_start={daily_start:.2f}, "
                             f"realized={realized:.2f}, target={target:.2f}")

                # Ganancia realizada
                if realized >= target:
                    logger.info(f"PTM TARGET_WIN (realizado): ${realized:.2f} >= ${target:.2f}")
                    return True, BailoutReason.TARGET_WIN
                if realized <= -1.5 * target:
                    logger.info(f"PTM TARGET_LOSS (realizado): ${realized:.2f}")
                    return True, BailoutReason.TARGET_LOSS

                # Balance combinado (incluye no realizado)
                if self.consider_unrealized:
                    summary = self._get_unrealized_summary()
                    if summary:
                        key      = 'combined_balance_net_est' if self.use_net_estimate else 'combined_balance_gross'
                        combined = summary.get(key)
                        if combined is not None:
                            combined_gain = float(combined) - daily_start
                            logger.debug(f"PTM: combined_gain={combined_gain:.2f}")
                            if combined_gain >= target:
                                logger.info(f"PTM TARGET_WIN (combinado): ${combined_gain:.2f} >= ${target:.2f}")
                                return True, BailoutReason.TARGET_WIN
                            if combined_gain <= -1.5 * target:
                                logger.info(f"PTM TARGET_LOSS (combinado): ${combined_gain:.2f}")
                                return True, BailoutReason.TARGET_LOSS

                return False, None

            except Exception as e:
                logger.exception(f"PTM: Error en is_target_reached: {e}")
                return False, None

    def should_bailout(self) -> tuple:
        """
        Evalua las 3 senales de bail-out inteligente.

        Returns:
            (bool, str)  — (True, razon) si debe salir, (False, '') si no.

        Las 3 señales:
          1. PEAK_DRAWDOWN       — el ciclo perdio X$ desde su mejor momento
          2. STAGNATION          — sin mejora en N minutos
          3. UNREALIZED_REVERSAL — el PnL no-realizado revertio demasiado

        Ninguna señal se activa antes de BAILOUT_MIN_CYCLE_MINUTES.
        """
        ...
        with self.lock:
            try:
                ...
                combined_gain = self._get_combined_gain()
                if combined_gain is None:
                    return False, ''

                # Actualizar estado del ciclo
                self._update_cycle_state(combined_gain)

                # Respetar tiempo minimo de ciclo
                if self._cycle_start_time is None:
                    return False, ''
                elapsed_min = (datetime.now() - self._cycle_start_time).total_seconds() / 60.0
                if elapsed_min < BAILOUT_MIN_CYCLE_MINUTES:
                    logger.debug(f"PTM bail-out: en ventana minima ({elapsed_min:.1f}/{BAILOUT_MIN_CYCLE_MINUTES} min)")
                    return False, ''

                # ── SEÑAL 1: Drawdown desde el pico ──────────────────────────
                if self._peak_combined_gain >= BAILOUT_PEAK_DRAWDOWN_MIN_PEAK_USD:
                    drawdown = self._peak_combined_gain - combined_gain
                    if drawdown >= BAILOUT_PEAK_DRAWDOWN_USD:
                        reason = (f"PEAK_DRAWDOWN | pico=${self._peak_combined_gain:.2f} | "
                                  f"actual=${combined_gain:.2f} | caida=${drawdown:.2f} "
                                  f"(limite=${BAILOUT_PEAK_DRAWDOWN_USD:.2f})")
                        logger.warning(f"PTM BAIL-OUT [{BailoutReason.PEAK_DRAWDOWN}]: {reason}")
                        self._last_bailout_eval = BailoutReason.PEAK_DRAWDOWN
                        return True, BailoutReason.PEAK_DRAWDOWN

                # ── SEÑAL 2: Estancamiento ────────────────────────────────────
                window_needed = int((BAILOUT_STAGNATION_MINUTES * 60) // max(BAILOUT_CHECK_INTERVAL_SEC, 1))
                if len(self._gain_history) >= max(window_needed, 3):
                    # La muestra mas antigua dentro de la ventana
                    cutoff_time = datetime.now() - timedelta(minutes=BAILOUT_STAGNATION_MINUTES)
                    old_samples = [(t, g) for t, g in self._gain_history if t <= cutoff_time]
                    if old_samples:
                        oldest_gain = old_samples[-1][1]   # El mas reciente dentro del pasado
                        improvement = combined_gain - oldest_gain
                        stagnation_active = improvement < BAILOUT_STAGNATION_MIN_DELTA_USD

                        # Condicion opcional: solo si la ganancia actual es negativa
                        if BAILOUT_STAGNATION_ONLY_IF_NEGATIVE:
                            stagnation_active = stagnation_active and (combined_gain < 0.0)

                        if stagnation_active:
                            reason = (f"STAGNATION | hace {BAILOUT_STAGNATION_MINUTES}min=${oldest_gain:.2f} | "
                                      f"actual=${combined_gain:.2f} | mejora=${improvement:.2f} "
                                      f"(minimo=${BAILOUT_STAGNATION_MIN_DELTA_USD:.2f})")
                            logger.warning(f"PTM BAIL-OUT [{BailoutReason.STAGNATION}]: {reason}")
                            self._last_bailout_eval = BailoutReason.STAGNATION
                            return True, BailoutReason.STAGNATION

                # ── SEÑAL 3: Reversión del PnL no-realizado ───────────────────
                if self._peak_unrealized >= BAILOUT_UNREALIZED_MIN_PEAK_USD:
                    current_unrealized = self._get_current_unrealized()
                    reversal_usd = self._peak_unrealized - current_unrealized
                    reversal_pct = (reversal_usd / self._peak_unrealized) * 100.0
                    if reversal_pct >= BAILOUT_UNREALIZED_REVERSAL_PCT:
                        reason = (f"UNREALIZED_REVERSAL | pico_unrealized=${self._peak_unrealized:.2f} | "
                                  f"actual_unrealized=${current_unrealized:.2f} | "
                                  f"reversion={reversal_pct:.1f}% (limite={BAILOUT_UNREALIZED_REVERSAL_PCT:.0f}%)")
                        logger.warning(f"PTM BAIL-OUT [{BailoutReason.UNREALIZED_REVERSAL}]: {reason}")
                        self._last_bailout_eval = BailoutReason.UNREALIZED_REVERSAL
                        return True, BailoutReason.UNREALIZED_REVERSAL

                logger.debug(f"PTM bail-out: ninguna señal activa | "
                             f"gain={combined_gain:.2f} | peak={self._peak_combined_gain:.2f} | "
                             f"unrealized_peak={self._peak_unrealized:.2f} | elapsed={elapsed_min:.1f}min")
                return False, ''

            except Exception as e:
                logger.exception(f"PTM: Error en should_bailout: {e}")
                return False, ''

    def get_cycle_status(self) -> Dict:
        """
        Devuelve un snapshot del estado interno del ciclo para mostrar en logs / UI.
        """
        with self.lock:
            try:
                combined_gain = self._get_combined_gain()
                ...
                if self._cycle_start_time:
                    elapsed_min = (datetime.now() - self._cycle_start_time).total_seconds() / 60.0
                drawdown = max(0.0, self._peak_combined_gain - (combined_gain or 0.0))
                return {
                    'combined_gain'      : combined_gain,
                    'peak_combined_gain' : self._peak_combined_gain,
                    'drawdown_from_peak' : drawdown,
                    'peak_unrealized'    : self._peak_unrealized,
                    'current_unrealized' : self._get_current_unrealized(),
                    'elapsed_minutes'    : elapsed_min,
                    'history_samples'    : len(self._gain_history),
                    'last_bailout_signal': self._last_bailout_eval,
                    'bailout_enabled'    : BAILOUT_ENABLED,
                }
            except Exception as e:
                logger.debug(f"PTM: Error en get_cycle_status: {e}")
                return {}
    ...
    def analyze_heikin_ashi_signal(self, symbol: str) -> Dict:
        ...
        try:
            ...
            if self.inversion_posiciones:
                ...
                if self.check_long_entry(last_bar):
                    signal_type = "LONG"
                elif self.check_short_entry(last_bar):
                    signal_type = "SHORT"
                else:
                    return {}
...
# ---------------------------------------------------------------------------------
    ...
    def check_exit_and_update(self, symbol: str):
        ...
        try:
            ...
            if trade.is_recovery_mode:
                ...
                if self.profit_target_manager.is_target_reached()[0]:
                ...
                # MODO NORMAL: Condiciones de cierre estándar
                    if trade.trade_type == "LONG":
                        if self.check_long_exit(last_barLOP) or current_roi < ROI_CRITICAL_LOSS:
    ...
    def compute_unrealized_pnl_summary(self) -> Dict:
        ...
        try:
            ...
            for symbol in active_symbols:
                ...
                # --- PYRAMIDING FIX: usar total_quantity y weighted_avg_price si están disponibles ---
                # Cuando hay pyramiding activo, trade.total_quantity y trade.weighted_avg_price
                # acumulan todas las entradas. Usar los originales (quantity / entry_price)
                # subestima el PnL real porque ignora las posiciones añadidas.
                raw_qty   = getattr(trade, 'total_quantity', None)
                raw_entry = getattr(trade, 'weighted_avg_price', None)

                # Fallback al trade original si los campos de pyramiding están vacíos/cero
                if raw_qty and float(raw_qty) > 0:
                    qty = float(raw_qty)
                else:
                    qty = float(getattr(trade, 'quantity', 0.0) or 0.0)

                if raw_entry and float(raw_entry) > 0:
                    entry = float(raw_entry)
                else:
                    entry = float(getattr(trade, 'entry_price', 0.0))
                # ---------------------------------------------------------------------------------

                # PnL bruto (sobre la cantidad total y precio promedio ponderado)
                if side == "LONG":
                    pnl_gross = (current_price - entry) * qty
                else:
                    pnl_gross = (entry - current_price) * qty

                # Fees estimados sobre la posición total
                try:
                ...
                # ROI respecto al capital invertido (entry promedio * qty total)
    ...
    def show_status(self):
        ...
        # Mostrar estado del bail-out
        if BAILOUT_ENABLED:
            bs = self.profit_target_manager.get_cycle_status()
            if bs:
                logger.info(
                    f"🚪 Bail-out | gain={bs.get('combined_gain', 0):.2f} | "
                    f"peak={bs.get('peak_combined_gain', 0):.2f} | "
                    f"drawdown={bs.get('drawdown_from_peak', 0):.2f}/${BAILOUT_PEAK_DRAWDOWN_USD:.2f} | "
                    f"unreal_peak={bs.get('peak_unrealized', 0):.2f} | "
                    f"elapsed={bs.get('elapsed_minutes', 0):.1f}min"
                )
    ...
    def profit_target_monitor_thread(self):
        """
        Monitor de profit targets + bail-out inteligente.

        Cada BAILOUT_CHECK_INTERVAL_SEC segundos evalua en orden:
          1. Cooldown activo           → esperar
          2. is_target_reached()       → cierre por ganancia/perdida extrema
          3. should_bailout()          → cierre preventivo (3 señales)

        El cierre es siempre ordenado: pausa entradas, espera que las posiciones
        abiertas se cierren naturalmente via check_exit_and_update, luego
        configura el cooldown y reinicia el tracking del ciclo.
        """
        logger.info("Iniciando monitor de profit targets + bail-out inteligente...")
        ...
        logger.info(f"   Bail-out activo   : {BAILOUT_ENABLED}")
        if BAILOUT_ENABLED:
            logger.info(f"   Peak drawdown     : ${BAILOUT_PEAK_DRAWDOWN_USD:.2f} (min pico ${BAILOUT_PEAK_DRAWDOWN_MIN_PEAK_USD:.2f})")
            logger.info(f"   Estancamiento     : {BAILOUT_STAGNATION_MINUTES}min sin +${BAILOUT_STAGNATION_MIN_DELTA_USD:.2f}")
            logger.info(f"   Reversión unreal  : {BAILOUT_UNREALIZED_REVERSAL_PCT:.0f}% desde pico (min pico ${BAILOUT_UNREALIZED_MIN_PEAK_USD:.2f})")
            logger.info(f"   Tiempo minimo     : {BAILOUT_MIN_CYCLE_MINUTES} min")

        # Iniciar tracking del primer ciclo
        self.profit_target_manager.reset_cycle_tracking()
                ...
                if self.is_in_cooldown():
                    ...
                    if remaining > 0:
                        if int(remaining) % 3600 == 0:
                            logger.info(f"En cooldown: {remaining / 3600:.1f}h restantes")
                        time.sleep(BAILOUT_CHECK_INTERVAL_SEC)
                        continue

                # ── 2. Target de ganancia / perdida extrema ─────────────────
                target_hit, target_reason = self.profit_target_manager.is_target_reached()
                if target_hit:
                    if target_reason == BailoutReason.TARGET_WIN:
                        logger.warning("=" * 60)
                        logger.warning("TARGET ALCANZADO — Protocolo de cierre ordenado")
                        logger.warning("=" * 60)
                    else:
                        logger.warning("=" * 60)
                        logger.warning(f"SALIDA POR PERDIDA EXTREMA [{target_reason}]")
                        logger.warning("=" * 60)
                    self._execute_ordered_closure(reason=target_reason)
                    time.sleep(5)
                    continue

                # ── 3. Bail-out inteligente ─────────────────────────────────
                if BAILOUT_ENABLED:
                    bail, bail_reason = self.profit_target_manager.should_bailout()
                    if bail:
                        logger.warning("=" * 60)
                        logger.warning(f"BAIL-OUT ACTIVADO [{bail_reason}]")
                        logger.warning("=" * 60)
                        self._execute_ordered_closure(reason=f"BAILOUT_{bail_reason}")
                        time.sleep(5)
                        continue

                # ── Log periódico del estado del ciclo ─────────────────────
                status = self.profit_target_manager.get_cycle_status()
                if status:
                    active_n = len(self.state_manager.get_all_active_symbols())
                    logger.debug(
                        f"PTM status | gain={status.get('combined_gain', 0):.2f} | "
                        f"peak={status.get('peak_combined_gain', 0):.2f} | "
                        f"drawdown={status.get('drawdown_from_peak', 0):.2f} | "
                        f"unreal_peak={status.get('peak_unrealized', 0):.2f} | "
                        f"elapsed={status.get('elapsed_minutes', 0):.1f}min | "
                        f"activos={active_n}"
                    )
    ...
    def _execute_ordered_closure(self, reason: str):
        """
        Cierre ordenado compartido por TARGET y BAIL-OUT.
        1. Pausa nuevas entradas.
        2. Envía ordenes de cierre para todas las posiciones activas.
        3. Espera confirmacion (max 5 min).
        4. Actualiza target, balance base, activa cooldown y reinicia tracking.
        """
        logger.info(f"Iniciando cierre ordenado | Razon: {reason}")

        # A. Pausar entradas nuevas
        self.pause_trading(seconds=300)

        # B. Enviar ordenes de cierre para todas las posiciones abiertas
        active_symbols = list(self.state_manager.get_all_active_symbols())
        logger.info(f"Posiciones a cerrar: {len(active_symbols)}")
        for symbol in active_symbols:
            try:
                trade = self.state_manager.get_trade(symbol)
                if trade and not self.state_manager.is_closing(symbol):
                    ...
                    self.order_executor.submit_command(cmd)
                    logger.info(f"   Cierre enviado: {symbol} [{reason}]")
            except Exception as e:
                logger.error(f"   Error enviando cierre para {symbol}: {e}")

        # C. Esperar cierre efectivo (max 5 minutos)
        max_wait = 300
        start    = time.time()
        while time.time() - start < max_wait:
            ...
            if not remaining:
                ...
                break
            logger.debug(f"   Esperando cierre de {len(remaining)} posicion(es)...")
            time.sleep(2)
        else:
            still_open = list(self.state_manager.get_all_active_symbols())
            if still_open:
                logger.warning(f"Timeout: forzando limpieza de {len(still_open)} posicion(es)")
                for sym in still_open:
                    try:
                        self.state_manager.cleanup_symbol(sym)
                    except Exception:
                        pass

        # D. Actualizar estado administrativo
        self.profit_target_manager.set_next_target()
        self.daily_start_balance = self.balance
        logger.info(f"Nuevo balance base: ${self.daily_start_balance:.2f}")

        # E. Activar cooldown
        self.set_cooldown(hours=self.profit_target_manager.wait_hours)

        # F. Reiniciar tracking del ciclo para el proximo
        self.profit_target_manager.reset_cycle_tracking()

        logger.info(f"Cierre ordenado completado [{reason}] — Bot en reposo.")
    ...
    def run(self):
        ...
        # Inicializar tracking de ciclo para bail-out
        self.profit_target_manager.reset_cycle_tracking()

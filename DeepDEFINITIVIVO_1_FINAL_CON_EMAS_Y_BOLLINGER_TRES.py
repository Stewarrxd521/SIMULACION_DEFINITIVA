    ...
    def is_target_reached(self) -> bool:
        ...
        with self.lock:
            try:
                ...
                if realized_gain >= target or realized_gain <= -1.5*target:  # Considerar también pérdida extrema
                ...
                if self.consider_unrealized:
                    ...
                    if summary:
                        ...
                        if combined is not None:
                            ...
                            if combined_gain >= target or combined_gain <= -1.5*target:  # Considerar también pérdida extrema
    ...
    def analyze_heikin_ashi_signal(self, symbol: str) -> Dict:
        ...
        try:
            ...
            if self.inversion_posiciones:
                if self.check_short_entry(last_bar)  and last_bar['open'] < last_bar['ema_20'] and last_bar['ema_20'] < last_bar['ema_70'] and last_bar['williams_r'] < -60:
                    signal_type = "LONG"
                elif self.check_long_entry(last_bar) and last_bar['open'] > last_bar['ema_20'] and last_bar['ema_20'] > last_bar['ema_70'] and last_bar['williams_r'] > -40:
                    signal_type = "SHORT"
                else:
                    return {}
            else:
    ...
    def compute_unrealized_pnl_summary(self) -> Dict:
        ...
        try:
            ...
            for symbol in active_symbols:
                ...
                qty = getattr(trade, 'quantity', 0.0) or 0.0
